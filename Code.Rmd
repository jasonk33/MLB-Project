...
par(xpd=TRUE)
barplot(rev(Home.Field.Advantage), names.arg = rev(Team.Names.Alphabetical), beside = FALSE, cex.names = 1.1, horiz = TRUE, las=1, space = 1, col = "slateblue3", border=NA)
mtext(text="Team", side=2, line=11, cex = 2)
mtext(text="Added Runs", side=1, line=2.5, cex = 2)
mtext(text="MLB Teams Home Field Advantage", side=3, line=.2, cex = 3)
par(xpd=TRUE)
```
...
sd(Home_Away_Difference$`Home Team Margin of Victory`)
...
mtext(text="Importance of Features in Predicting Home Score", side=3, line=.7, cex = 3)

legend(1.57, 48.6, "Important", fill = "Green")

legend(1.57, 38.6, "Unimportant", fill = "Red")

legend(1.57, 28.6, "Undecided", fill = "Yellow")

legend(1.57, 18.6, "Random Predictor", fill = "Blue")
```
...
colnames(Mean.Offense)[1:16]=c("Team", "Batting Average", "Slugging Percentage", "On Base Percentage", "OPS", "Score", "Home Mean Batting Average", "Away Mean Batting Average", "Home Mean Slugging Percentage", "Away Mean Slugging Percentage", "Home Mean On Base Percentage", "Away Mean On Base Percentage", "Home Mean OPS", "Away Mean OPS", "Home Mean Score", "Away Mean Score")
...
Mean.Offense$`On Base Percentage` <- (Mean.Offense$`Home Mean On Base Percentage` + Mean.Offense$`Away Mean On Base Percentage`)/2
...
par(mar=c(3, 1, 2.5, 0))
parcoord(Mean.Offense[, c(2, 3, 4, 5, 6)], col=ifelse(Mean.Offense$`Score` > 4.31, 3, ifelse(Mean.Offense$`Score` < 4.07, 2, 1)), var.label = TRUE, lwd = 15)
mtext(text="Offensive Variables Determining Score", side=3, line=-.1, cex = 3)
```
...
par(mar=c(5, 5.5, 3, 1))
barplot(AL.Attendance.Boost$`Percent Increase`, names.arg = AL.Attendance.Boost$`Team`, ylab = "Percent Change", xlab = "Team", main = "Road Attendence Affect", col = "slateblue3", cex.names = .9, cex.main = 3, cex.lab = 2)
```
